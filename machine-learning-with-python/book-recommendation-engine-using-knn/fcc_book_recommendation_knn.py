...
users_to_remove = user_ratingCount.query(
    'totalRatingCount > 200').userId.tolist()
...
rating_with_totalRatingCount = df.merge(
    book_ratingCount, left_on='title', right_on='title', how='left')
pd.set_option('display.float_format', lambda x: '%.3f' % x)

# Remove books with less than 100 ratings
rating_popular_books = rating_with_totalRatingCount.query(
    'totalRatingCount > 100')

# remove from the dataset users with less than 200 ratings
rating_popular_books = rating_popular_books[rating_popular_books['userId'].isin(
    users_to_remove)]
...
def get_recommends(book=""):
    model_knn = NearestNeighbors(
        metric='cosine', n_neighbors=5, algorithm='auto')
    ...
    for i in range(1, len(distances.flatten())):
        ret[1].insert(
            0, [book_features_df.index[indices.flatten()[i]], distances.flatten()[i]])
    return ret
...
def test_book_recommendation():
    ...
    recommended_books = ["I'll Be Seeing You", 'The Weight of Water',
